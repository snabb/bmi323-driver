...
use bmi323_driver::{
    AccelConfig, ActiveLevel, Bmi323, FeatureBlockingMode, I2C_ADDRESS_PRIMARY, InterruptChannel,
    InterruptPinConfig, InterruptRoute, InterruptSource, OrientationConfig, OrientationMode,
    OutputDataRate, OutputMode,
};
...
async fn main(_spawner: Spawner) {
    ...
    if let Err(err) = imu
        .configure_orientation(OrientationConfig {
    ...
    loop {
        match imu
            ...
            .await
        {
            Ok(status) if status.orientation() => match imu.read_accel().await {
                Ok(accel) => {
                    let [x, y, z] = accel.as_g(imu.accel_range());
                    info!(
                        "orientation change accel_g=({=f32}, {=f32}, {=f32})",
                        x, y, z
                    );
                }
