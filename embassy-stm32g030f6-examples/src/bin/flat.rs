...
use bmi323_driver::{
    AccelConfig, AccelRange, ActiveLevel, Bmi323Async, FeatureBlockingMode, FlatConfig,
    I2C_ADDRESS_PRIMARY, InterruptChannel, InterruptPinConfig, InterruptRoute,
    InterruptSource, OutputDataRate, OutputMode,
};
...
async fn main(_spawner: Spawner) {
    ...
    loop {
        match imu
            ...
            .await
        {
            Ok(status) if status.flat() => match imu.read_accel().await {
                Ok(accel) => {
                    let [x, y, z] = accel.as_g(AccelRange::G8);
                    info!("flat detected accel_g=({=f32}, {=f32}, {=f32})", x, y, z);
                }
