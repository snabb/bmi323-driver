...
use bmi323_driver::{
    ...
    InterruptRoute, InterruptSource, OutputDataRate, OutputMode, StepCounterConfig,
};
...
async fn main(_spawner: Spawner) {
    ...
    if let Err(err) = imu.set_step_detector_enabled(true).await {
        defmt::panic!("BMI323 step-detector enable failed: {:?}", err);
    }

    if let Err(err) = imu.set_step_counter_enabled(true).await {
        defmt::panic!("BMI323 step-counter enable failed: {:?}", err);
    }
    ...
    if let Err(err) = imu
        ...
        .await
    {
        defmt::panic!("BMI323 step-counter config failed: {:?}", err);
    }
    ...
    if let Err(err) = imu
        ...
        .await
    {
        defmt::panic!("BMI323 step-detector routing failed: {:?}", err);
    }
    ...
    if let Err(err) = imu
        ...
        .await
    {
        defmt::panic!("BMI323 step-counter routing failed: {:?}", err);
    }
