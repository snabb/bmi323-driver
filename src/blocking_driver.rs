...
use crate::{
    ...
    SignificantMotionConfig, StatusWord, StepCounterConfig, SyncAccess, TapConfig, TiltConfig,
};
    ...
    Self: SyncAccess,
{
    ...
    /// gyroscope explicitly with [`set_accel_config`](Self::set_accel_config)
    /// and [`set_gyro_config`](Self::set_gyro_config) before depending on
    /// sample reads in application code.
    pub fn init<D: DelayNs>(
        &mut self,
        delay: &mut D,
    ) -> Result<DeviceState, Error<<Self as SyncAccess>::BusError>> {
        ...
        if matches!(self.kind, TransportKind::Spi) {
            self.read_word(CHIP_ID).map_err(Error::Bus)?;
            delay.delay_us(250);
        }

        let chip_id = self.read_word(CHIP_ID).map_err(Error::Bus)? as u8;
        if chip_id != BMI323_CHIP_ID {
            return Err(Error::InvalidChipId(chip_id));
        }

        let error = self.error_word()?;
        if error.fatal() {
    ...
    pub fn run_self_test<D: DelayNs>(
        &mut self,
        delay: &mut D,
        selection: SelfTestSelection,
    ) -> Result<SelfTestResult, Error<<Self as SyncAccess>::BusError>> {
        let saved_acc_conf = self.read_word(ACC_CONF).map_err(Error::Bus)?;
        let saved_gyr_conf = self.read_word(GYR_CONF).map_err(Error::Bus)?;
        let saved_alt_acc_conf = self.read_word(ALT_ACC_CONF).map_err(Error::Bus)?;
        let saved_alt_gyr_conf = self.read_word(ALT_GYR_CONF).map_err(Error::Bus)?;
        let saved_st_select = self.read_feature_word(EXT_ST_SELECT)?;
        let saved_accel_range = self.accel_range;
        let saved_gyro_range = self.gyro_range;

        let result = self.run_self_test_inner(delay, selection);
        let restore = self.restore_self_test_configuration(
            saved_acc_conf,
            saved_gyr_conf,
            saved_alt_acc_conf,
            saved_alt_gyr_conf,
    ...
    pub fn status_word(&mut self) -> Result<StatusWord, Error<<Self as SyncAccess>::BusError>> {
        self.read_word(STATUS).map(StatusWord).map_err(Error::Bus)
    }

    /// Read and decode the `ERR_REG` register.
    pub fn error_word(&mut self) -> Result<ErrorWord, Error<<Self as SyncAccess>::BusError>> {
        self.read_word(ERR_REG).map(ErrorWord).map_err(Error::Bus)
    }
    ...
    pub fn read_accel(&mut self) -> Result<AxisData, Error<<Self as SyncAccess>::BusError>> {
    ...
    pub fn read_gyro(&mut self) -> Result<AxisData, Error<<Self as SyncAccess>::BusError>> {
        let mut words = [0u16; 3];
    ...
    /// [`set_gyro_config`](Self::set_gyro_config).
    pub fn read_imu_data(&mut self) -> Result<ImuData, Error<<Self as SyncAccess>::BusError>> {
    ...
    pub fn read_temperature_celsius(
        &mut self,
    ...
    pub fn read_sensor_time(&mut self) -> Result<u32, Error<<Self as SyncAccess>::BusError>> {
    ...
    pub fn configure_interrupt_pin(
        ...
        let mut word = self.read_word(IO_INT_CTRL).map_err(Error::Bus)?;
        ...
        self.write_word(IO_INT_CTRL, word).map_err(Error::Bus)
    }

    /// Enable or disable latched interrupt behavior inside the sensor.
    pub fn set_interrupt_latching(
        &mut self,
        latched: bool,
    ) -> Result<(), Error<<Self as SyncAccess>::BusError>> {
    ...
    pub fn map_interrupt(
        ...
        let mut word = self.read_word(register).map_err(Error::Bus)?;
        word &= !(0b11 << shift);
        word |= u16::from(route) << shift;
        self.write_word(register, word).map_err(Error::Bus)
    }
    ...
    pub fn read_fifo_words(
        ...
        self.read_words(FIFO_DATA, words).map_err(Error::Bus)
    }
    ...
    pub fn enable_feature_engine(&mut self) -> Result<(), Error<<Self as SyncAccess>::BusError>> {
        ...
        self.write_word(FEATURE_IO2, 0x012C).map_err(Error::Bus)?;
        self.write_word(FEATURE_IO_STATUS, 0x0001)
            .map_err(Error::Bus)?;
        self.write_word(FEATURE_CTRL, 0x0001).map_err(Error::Bus)?;
    ...
    pub fn configure_flat(
        ...
        self.set_flat_enabled(true)
    }

    /// Enable or disable the flat-detection feature.
    pub fn set_flat_enabled(
        &mut self,
        enabled: bool,
    ) -> Result<(), Error<<Self as SyncAccess>::BusError>> {
        self.modify_word(FEATURE_IO0, |mut word| {
    ...
    pub fn configure_orientation(
        ...
        self.set_orientation_enabled(true)
    }

    /// Enable or disable the orientation-detection feature.
    pub fn set_orientation_enabled(
        &mut self,
        enabled: bool,
    ) -> Result<(), Error<<Self as SyncAccess>::BusError>> {
        self.modify_word(FEATURE_IO0, |mut word| {
    ...
    pub fn configure_significant_motion(
        &mut self,
        ...
        self.set_significant_motion_enabled(true)
    }

    /// Enable or disable the significant-motion feature.
    pub fn set_significant_motion_enabled(
        &mut self,
        enabled: bool,
    ) -> Result<(), Error<<Self as SyncAccess>::BusError>> {
        self.modify_word(FEATURE_IO0, |mut word| {
    ...
    pub fn configure_tilt(
        ...
        self.set_tilt_enabled(true)
    }

    /// Enable or disable the tilt-detection feature.
    pub fn set_tilt_enabled(
        &mut self,
        enabled: bool,
    ) -> Result<(), Error<<Self as SyncAccess>::BusError>> {
        self.modify_word(FEATURE_IO0, |mut word| {
    ...
    pub fn set_step_detector_enabled(
        &mut self,
        enabled: bool,
    ) -> Result<(), Error<<Self as SyncAccess>::BusError>> {
        self.modify_word(FEATURE_IO0, |mut word| {
    ...
    pub fn set_step_counter_enabled(
        &mut self,
        enabled: bool,
    ) -> Result<(), Error<<Self as SyncAccess>::BusError>> {
        self.modify_word(FEATURE_IO0, |mut word| {
    ...
    pub fn configure_step_counter(
        &mut self,
        config: StepCounterConfig,
    ) -> Result<(), Error<<Self as SyncAccess>::BusError>> {
        self.write_feature_word(EXT_SC_1, config.to_word())
    }

    /// Request a reset of the accumulated step count.
    pub fn reset_step_counter(&mut self) -> Result<(), Error<<Self as SyncAccess>::BusError>> {
    ...
    pub fn read_step_count(&mut self) -> Result<u32, Error<<Self as SyncAccess>::BusError>> {
        let low = self.read_word(FEATURE_IO2).map_err(Error::Bus)? as u32;
        let high = self.read_word(FEATURE_IO3).map_err(Error::Bus)? as u32;
        Ok(low | (high << 16))
    }

    /// Program the alternate accelerometer configuration in `ALT_ACC_CONF`.
    pub fn set_alt_accel_config(
        &mut self,
    ...
    /// configurations.
    pub fn configure_alt_config_control(
        &mut self,
    ...
    pub fn alt_status(&mut self) -> Result<AltStatus, Error<<Self as SyncAccess>::BusError>> {
        self.read_word(ALT_STATUS)
            .map(AltStatus)
            .map_err(Error::Bus)
    }

    fn modify_word<F>(&mut self, reg: u8, f: F) -> Result<(), Error<<Self as SyncAccess>::BusError>>
    where
        F: FnOnce(u16) -> u16,
    {
        let value = self.read_word(reg).map_err(Error::Bus)?;
        self.write_word(reg, f(value)).map_err(Error::Bus)?;
        if reg == FEATURE_IO0 {
            self.write_word(FEATURE_IO_STATUS, 1).map_err(Error::Bus)?;
        }
        Ok(())
    }

    fn run_self_test_inner<D: DelayNs>(
        &mut self,
        delay: &mut D,
        selection: SelfTestSelection,
    ) -> Result<SelfTestResult, Error<<Self as SyncAccess>::BusError>> {
        ...
        self.write_word(ALT_ACC_CONF, 0).map_err(Error::Bus)?;
        self.write_word(ALT_GYR_CONF, 0).map_err(Error::Bus)?;
    ...
    fn restore_self_test_configuration(
        &mut self,
        acc_conf: u16,
        gyr_conf: u16,
        alt_acc_conf: u16,
        ...
        self.write_word(ACC_CONF, acc_conf).map_err(Error::Bus)?;
        self.write_word(GYR_CONF, gyr_conf).map_err(Error::Bus)?;
        self.write_word(ALT_ACC_CONF, alt_acc_conf)
            .map_err(Error::Bus)?;
        self.write_word(ALT_GYR_CONF, alt_gyr_conf)
            .map_err(Error::Bus)?;
        self.write_feature_word(EXT_ST_SELECT, st_select)
    }

    fn write_feature_word(
        &mut self,
        ext_addr: u16,
        value: u16,
    ) -> Result<(), Error<<Self as SyncAccess>::BusError>> {
        self.write_word(FEATURE_DATA_ADDR, ext_addr)
            .map_err(Error::Bus)?;
        self.write_word(FEATURE_DATA_TX, value).map_err(Error::Bus)
    }

    fn read_feature_word(
        &mut self,
        ext_addr: u16,
    ) -> Result<u16, Error<<Self as SyncAccess>::BusError>> {
        self.write_word(FEATURE_DATA_ADDR, ext_addr)
            .map_err(Error::Bus)?;
        self.read_word(FEATURE_DATA_TX).map_err(Error::Bus)
    }

    fn modify_feature_word<F>(
        &mut self,
        ext_addr: u16,
        f: F,
    ) -> Result<(), Error<<Self as SyncAccess>::BusError>>
    where
        F: FnOnce(u16) -> u16,
    {
        let word = self.read_feature_word(ext_addr)?;
        self.write_feature_word(ext_addr, f(word))
    }
    ...
    fn apply_common_feature_settings(
        &mut self,
